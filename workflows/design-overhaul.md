...
2. Rebuild the gallery markup/CSS: 9 square tiles in 3 rows (2x3 on mobile); each tile is an `overflow:hidden` window over a strip of stacked photos. The owner then asked for a *visible* transition (not an instant swap), so the shipped version is a hold-and-glide keyframe animation, not `steps()`: the strip holds each photo ~2.9s and eases 0.6s to the next. For a seamless loop the strip carries its 3 photos **plus a duplicate of the first at the end** (4 frames, each 25% of strip height; keyframes hold at 0 / -25% / -50% and end at -75%, which is pixel-identical to the start). Easing declared *on a keyframe* applies to the segment leaving it — put it on the hold-end keyframes to ease exactly the slides. `animation-direction: reverse` on row 2 both flips the glide direction and staggers its slide moments relative to rows 1/3, which reads livelier than lockstep.
...
- **Production CSP has no `'unsafe-inline'` for scripts** (`vercel.json`: `script-src 'self' https://www.googletagmanager.com`). Any inline `<script>` in the HTML executes fine on a local dev server (which sends no CSP) and is silently dead on the live site — this is exactly how the day/night time-based default "worked locally, broken in production" (2026-07-17). Never ship inline scripts in this repo; put the logic in an external `assets/js/*.js` file. Corollary: local Playwright screenshots do NOT prove production behavior for anything script-related — when a "works here, not there" report comes in, drive the *production* URL headlessly and read the console (the CSP violation message names the blocked directive outright).
- The site auto-deploys on push to `master` (verified 2026-07-17: `styles.css?v=15` was live minutes after the push, no manual `vercel deploy` involved) — the older memory that the Vercel CLI is the only reliable deploy path is out of date for plain pushes; the CLI is still the tool for env vars and instant/forced deploys.
